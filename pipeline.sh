...
# Years to pull. Override by passing as arguments.
YEARS=("${1:-2000}")
...
for YEAR in "${YEARS[@]}"; do
    ...
    if [ -f "$RAW_GZ" ]; then
            echo "⏭️  Already downloaded: $RAW_GZ"
        else
            echo "⬇️  Downloading $RAW_GZ ..."
            curl -L --progress-bar -o "$RAW_GZ" "$URL"

            # Guard against failed downloads (HTML error pages are small)
            FILE_SIZE=$(wc -c < "$RAW_GZ" | tr -d ' ')
            if [ "$FILE_SIZE" -lt 10000 ]; then
                echo "❌ ERROR: $RAW_GZ is only $FILE_SIZE bytes — download likely failed."
    ...
    rm -f "$OUT_PARQUET"  # Remove existing file first — Parquet driver can't overwrite in place
    ogr2ogr \
        -f "Parquet" \
        "$OUT_PARQUET" \
        "$RAW_CSV" \
    ...
    PARQUET_SIZE=$(wc -c < "$OUT_PARQUET" | tr -d ' ')
